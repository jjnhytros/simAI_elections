...
def generate_random_event(candidates_info, elector_full_preferences_data, last_round_results=None):
    ...
    if chosen_event == "scandal" and candidates_info:
        # Trova il candidato con l'integrità più bassa (più probabile) o casuale
        scandalized_cand = min(
            candidates_info, key=lambda c: c['attributes'].get('ethical_integrity', 5))
        if random.random() < 0.7:  # Maggiore probabilità per il meno integro
            scandalized_name = scandalized_cand['name']
            scandal_impact = random.uniform(1.0, 3.0) * (6 - scandalized_cand['attributes'].get(
                'ethical_integrity', 5))  # Impatto maggiore per bassa integrità
        else:  # Scandal casuale
            scandalized_name = random.choice(candidates_info)['name']
            scandal_impact = random.uniform(1.0, 3.0)

        utils.send_pygame_update(
            utils.UPDATE_TYPE_MESSAGE, f"  Scandalo su {scandalized_name}!")
        ...
        attr_focused = random.choice(
            ["administrative_experience", "social_vision", "mediation_ability", "ethical_integrity"])
        utils.send_pygame_update(
            utils.UPDATE_TYPE_MESSAGE, f"  Focus su {attr_focused.replace('_',' ').title()}!")
        for e_id in elector_full_preferences_data:
            e_data = elector_full_preferences_data.get(e_id)
            if not e_data:
                continue
            e_ideal_pref = e_data.get(
                f'preference_{attr_focused}', config.ELECTOR_IDEAL_PREFERENCE_RANGE[0])
            for cand in candidates_info:
                ...
                max_dist = config.ELECTOR_IDEAL_PREFERENCE_RANGE[1] - \
        ...
        attr_shifted = random.choice(
            ["administrative_experience", "social_vision", "mediation_ability", "ethical_integrity"])
        # -1 per shift verso basso, 1 per shift verso alto
        shift_dir = random.choice([-1, 1])
        shift_magnitude = random.uniform(0.5, 1.5)
        utils.send_pygame_update(
            utils.UPDATE_TYPE_MESSAGE, f"  Shift d'opinione su {attr_shifted.replace('_',' ').title()} ({'Alto' if shift_dir > 0 else 'Basso'})!")
        for e_id in elector_full_preferences_data:
            e_data = elector_full_preferences_data.get(e_id)
            if not e_data:
                continue
            ...
            for cand_name, cand_lean in e_current_leanings.items():
                cand_info = next(
                    (c for c in candidates_info if c['name'] == cand_name), None)
                if not cand_info:
                    continue
                ...
                if shift_dir > 0:  # Shift verso alto
                    impact = (cand_attr_val - config.ATTRIBUTE_RANGE[0]) / (
                        config.ATTRIBUTE_RANGE[1] - config.ATTRIBUTE_RANGE[0]) * shift_magnitude
        ...
        utils.send_pygame_update(
            utils.UPDATE_TYPE_MESSAGE, f"  Gaffe di {gaffe_name}!")
        ...
        utils.send_pygame_update(
            utils.UPDATE_TYPE_MESSAGE, f"  Successo di {success_name}!")
        for e_id in elector_full_preferences_data:
        ...
        utils.send_pygame_update(
            utils.UPDATE_TYPE_MESSAGE, f"  Dibattito Etico!")
        for e_id in elector_full_preferences_data:
            e_data = elector_full_preferences_data.get(e_id)
            if not e_data:
                continue
            ...
            for cand in candidates_info:
                ...
                norm_integrity = (cand_integrity - config.ATTRIBUTE_RANGE[0]) / (config.ATTRIBUTE_RANGE[1] - config.ATTRIBUTE_RANGE[0]) if (
                    config.ATTRIBUTE_RANGE[1] - config.ATTRIBUTE_RANGE[0]) > 0 else 0.5
                # Centra l'impatto intorno allo 0
                base_impact = (norm_integrity - 0.5) * \
        ...
        utils.send_pygame_update(
            utils.UPDATE_TYPE_MESSAGE, f"  Endorsement per {endorsed_name}!")
        for e_id in elector_full_preferences_data:
...
def execute_voting_round(current_voting_electors_ids, votable_candidates_info, elector_prefs_data, last_results=None, num_electors_base=0, req_maj_perc=0.6, current_round=0, all_cands_info=None, carryover_name=None, continue_event=None, running_event=None, step_mode=False):
    ...
    if last_results and num_electors_base > 0 and current_round > 0:
        ...
        for e_id in local_prefs:
            d = local_prefs[e_id]
            l = d.get(
                'leanings', {})  # Fixed: was 'l'
            t = d.get('traits', [])  # Fixed: was 't'
            mod = 1.0
            # Use full trait names for clarity and consistency with config
            if "Swing Voter" in t:  # Example trait check
                mod *= 1.5
            if "Loyal" in t:  # Example trait check
                mod *= 0.5
            if "Contrarian" in t:  # Example trait check
                mod *= -0.5  # Might react negatively to momentum? Example.
    ...
    if current_round == 1 and carryover_name is not None:
        ...
        for e_id in local_prefs:
            l = local_prefs[e_id].get('leanings', {})  # Fixed: was 'l'
...
def run_election_simulation(election_attempt=1, preselected_candidates_info=None, runoff_carryover_winner_name=None, continue_event=None, running_event=None, step_by_step_mode=False):
    ...
    # Init Budgets (Budgets are now loaded/initialized in generation.py and stored/updated in DB)
    # We just need to ensure the candidate objects in governor_cands_info have the correct current_budget
    # This is handled in generation.py now when candidates are created/loaded.
    utils.send_pygame_update(utils.UPDATE_TYPE_MESSAGE,
                             "\n--- Init Campaign Budgets (Loaded/Initialized from DB) ---")
    for cand in governor_cands_info:
        # Log the budget that was loaded/initialized in generation.py
        utils.send_pygame_update(
            utils.UPDATE_TYPE_MESSAGE, f"  {cand['name']}: Budget {cand.get('campaign_budget', 0):.2f}")  # Use .get for safety
    ...
    while governor_elected is None and current_round < config.MAX_TOTAL_ROUNDS:
        ...
        if current_round > 0:  # Campaigning starts from round 1 now
            utils.send_pygame_update(
            ...
            if campaign_cands:
                # --- Competitive Adaptation Step ---
                utils.send_pygame_update(
                    utils.UPDATE_TYPE_MESSAGE, " - Competitive Adaptation -")
                for cand in campaign_cands:
                    # Pass last_results for analysis
                    voting.analyze_competition_and_adapt_strategy(
                        cand, governor_cands_info, last_results, current_round)
                utils.send_pygame_update(utils.UPDATE_TYPE_MESSAGE, "-"*25)
                # Small pause for adaptation
        ...
        if governor_elected:
            ...
            # Update candidate stats in DB - Basic example: just mark as elected
            elected_cand_info = next(
                (c for c in governor_cands_info if c['name'] == governor_elected), None)
            if elected_cand_info:
                stats = elected_cand_info.get('stats', {})
                stats['elections_won'] = stats.get('elections_won', 0) + 1
                # Update in current object
                elected_cand_info['stats'] = stats
                # Need to call save_candidate to persist this to DB
                # For simplicity here, just passing relevant data. A full implementation would load the DB candidate object first.
                # Ensure all required fields are present when saving to DB
                db_manager.save_candidate({
                    # Ensure UUID is present
                    'uuid': elected_cand_info.get('uuid', str(uuid.uuid4())),                      'name': elected_cand_info['name'],
                    # Provide defaults if missing
                    'gender': elected_cand_info.get('gender', 'Unknown'),
                    'age': elected_cand_info.get('age', 0),
                    'party_id': elected_cand_info.get('party_id', 'Unknown'),
                    # Need initial budget in candidate obj or fetch from DB
                    'initial_budget': elected_cand_info.get('initial_budget', config.INITIAL_CAMPAIGN_BUDGET),
                    # Save final budget
                    'current_budget': elected_cand_info.get('campaign_budget', 0),
                    'attributes': elected_cand_info.get('attributes', {}),
                    'traits': elected_cand_info.get('traits', []),
                    'stats': stats  # Pass updated stats
                })

            utils.send_pygame_update(utils.UPDATE_TYPE_COMPLETE, {
                "elected": True, "governor": governor_elected, "attempt": election_attempt, "round": current_round})  # Added attempt and round
            if running_event:
                running_event.clear()
            break  # Exit loop on election
        else:
    ...
    if governor_elected:
        ...
        pass  # The logic for elected is above inside the loop
        ...
        if election_attempt < config.MAX_ELECTION_ATTEMPTS:
            ...
            if results_carry:
                ...
                if runoff_mode and len(runoff_cand_names) == 2:
                    ...
                    if sorted_runoff:
                        # In runoff deadlock, carry over BOTH candidates
                        top_names = [c[0] for c in sorted_runoff]
                        utils.send_pygame_update(
                            utils.UPDATE_TYPE_MESSAGE, f"Runoff Deadlock. Carrying over: {top_names[0]} & {top_names[1]}")
                        next_preselected = [
                            c for c in governor_cands_info if c['name'] in top_names]
                        # No single carryover winner with bonus in runoff deadlock
                        next_carryover_name = None

                else:  # General deadlock logic (not in runoff)
                    utils.send_pygame_update(
                    ...
                    next_preselected = [
                        c for c in governor_cands_info if c["name"] in top_names]
                    # If only one is carried over, give them the bonus in the next attempt
                    if len(next_preselected) == 1:
                        next_carryover_name = next_preselected[0]['name']
                        utils.send_pygame_update(
                            utils.UPDATE_TYPE_MESSAGE, f"{next_carryover_name} carried over w/ bonus.")
                    else:
                        next_carryover_name = None  # No bonus if multiple or none carried

                # --- Update candidate stats in DB for non-elected candidates (basic example: increment elections contested) ---
                contested_cands = [c for c in governor_cands_info if c['name'] not in (
                    governor_elected if governor_elected else '')]
                for cand in contested_cands:
                    stats = cand.get('stats', {})
                    stats['elections_contested'] = stats.get(
                        'elections_contested', 0) + 1
                    cand['stats'] = stats  # Update in current object
                    # Save to DB - need to fetch full data or ensure object has all necessary fields
                    # Ensure all required fields are present when saving to DB
                    db_manager.save_candidate({
                        'uuid': cand.get('uuid', str(uuid.uuid4())),
                        'name': cand['name'],
                        'gender': cand.get('gender', 'Unknown'),
                        'age': cand.get('age', 0),
                        'party_id': cand.get('party_id', 'Unknown'),
                        # Need initial budget in candidate obj or fetch
                        'initial_budget': cand.get('initial_budget', config.INITIAL_CAMPAIGN_BUDGET),
                        # Save final budget
                        'current_budget': cand.get('campaign_budget', 0),
