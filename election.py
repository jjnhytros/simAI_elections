...
def execute_district_election(district_id,
    ...
    if continue_event and step_by_step_mode:
        utils.send_pygame_update(
            utils.UPDATE_TYPE_STATUS, {
                ...
                "round": district_id,
                "status": f"District {district_id} - Waiting..."
            })
        time.sleep(0.05)
        continue_event.wait()
    ...
    utils.send_pygame_update(
        utils.UPDATE_TYPE_STATUS, {
            ...
            "round": district_id,
            "status": f"Distr {district_id} Voting"
        })
    dist_cands = generation.generate_candidates(config.CANDIDATES_PER_DISTRICT,
    ...
    dist_votes = [
        voting.simulate_citizen_vote(cid, dist_cands, cinfo)
        for cid, cinfo in citizens.items()
    ]
    ...
    if dist_results:
        if winners_to_elect > 0:
            ...
            if actual_elected < winners_to_elect:
                utils.send_pygame_update(
                    utils.UPDATE_TYPE_MESSAGE,
                    f"    Warn: Only {actual_elected}/{winners_to_elect} elected in Distr {district_id}."
                )
            winners_names = set(
                c[0] for c in dist_results.most_common(actual_elected))
            winners_info = [
                c for c in dist_cands if c["name"] in winners_names
            ]
        else:
            utils.send_pygame_update(utils.UPDATE_TYPE_MESSAGE,
                                     f"    Distr {district_id} elects 0.")
    else:
        utils.send_pygame_update(
            utils.UPDATE_TYPE_MESSAGE,
            f"    No valid votes in Distr {district_id}.")
    # GUI Results
    gui_res = []
...
def execute_voting_round(current_voting_electors_ids,
    ...
    if last_results and num_electors_base > 0 and current_round > 0:
        ...
        cands = list(
            local_prefs[ref_key]['leanings'].keys()) if ref_key else []
        if cands:
            avg = 1.0 / len(cands)
        for e_id in local_prefs:
            d = local_prefs[e_id]
            l = d.get('leanings', {})  # Fixed: was 'l'
        ...
        for e_id in local_prefs:
            ...
            if carryover_name in l:
                l[carryover_name] = max(
                    0.1,
                    l[carryover_name] + config.RUNOFF_CARRYOVER_LEANING_BONUS)
    ...
    for e_id in current_voting_electors_ids:
        ...
        if not e_data:
            continue
        vote = voting.simulate_ai_vote(e_id, votable_candidates_info, e_data,
                                       last_results, current_round,
                                       all_cands_info)
        if vote is not None:
...
def run_election_simulation(election_attempt=1,
    ...
    if num_preselected > 0:
        ...
        if needed_from_districts < 0:
            utils.send_pygame_update(utils.UPDATE_TYPE_ERROR,
                                     "Config Error: Neg winners needed")
        ...
        if needed_from_districts > 0:
            if winners_extra > 0:
                utils.send_pygame_update(
                    utils.UPDATE_TYPE_MESSAGE,
                    f"{winners_extra} D elect {winners_base+1}, {config.NUM_DISTRICTS-winners_extra} elect {winners_base}."
                )
            else:
                utils.send_pygame_update(utils.UPDATE_TYPE_MESSAGE,
                                         f"Each D elects {winners_base}.")
        else:
            utils.send_pygame_update(utils.UPDATE_TYPE_MESSAGE,
                                     "Phase 1 skipped.")
    else:
    ...
    if needed_from_districts > 0:
        for i in range(1, config.NUM_DISTRICTS + 1):
            ...
            dist_winners = execute_district_election(i, winners_this_dist,
                                                     continue_event,
                                                     running_event,
                                                     step_by_step_mode)
    ...
    if len(governor_cands_info) != config.NUM_GRAND_ELECTORS:  # Final Check
        utils.send_pygame_update(
            ...
            f"Internal Error: Final cands ({len(governor_cands_info)}) != NUM_GRAND_ELECTORS ({config.NUM_GRAND_ELECTORS})."
        )
        utils.send_pygame_update(utils.UPDATE_TYPE_COMPLETE,
                                 {"elected": False})
        if running_event:
    ...
    for cand in governor_cands_info:
        ...
        # Initialize campaign themes (can be adapted later)
        attrs = cand.get('attributes', {})
        sorted_attrs = sorted(attrs.items(), key=lambda i: i[1], reverse=True)
        # Initial themes are just based on candidate's top attributes
        cand['current_campaign_themes'] = [
            a[0] for a in sorted_attrs[:random.randint(1, 2)]
        ]
        utils.send_pygame_update(
            utils.UPDATE_TYPE_MESSAGE,
            f"  {cand['name']}: Budget {cand['campaign_budget']:.2f}")
    ...
    for i, cand in enumerate(gov_cands_sorted):
        ...
        attrs = cand.get("attributes", {})
        attrs_str = ", ".join([f"{k[:3]}:{v}"
                               for k, v in attrs.items()])  # Abbreviato
        utils.send_pygame_update(
            utils.UPDATE_TYPE_MESSAGE,
            f"  Age: {cand.get('age','?')} | Attrs: {attrs_str if attrs_str else 'N/A'}"
        )
        utils.send_pygame_update(
            utils.UPDATE_TYPE_MESSAGE,
            f'  Stmt: "{generate_candidate_oath(cand, config.CURRENT_HOT_TOPIC)}"'
        )
    ...
    while governor_elected is None and current_round < config.MAX_TOTAL_ROUNDS:
        ...
        if current_round > 0:  # Campaigning starts from round 1 now
            ...
            if campaign_cands:
                # --- Competitive Adaptation Step (NUOVO) ---
                utils.send_pygame_update(utils.UPDATE_TYPE_MESSAGE,
                ...
                for cd in governor_cands_info:
                    utils.send_pygame_update(
                        utils.UPDATE_TYPE_MESSAGE,
                        f"  {cd['name']}: {cd.get('campaign_budget', 0):.2f}")
                # Run campaign
                voting.simulate_campaigning(campaign_cands, grand_electors,
                                            current_attempt_prefs,
                                            last_results)
                # Log budget post
                utils.send_pygame_update(utils.UPDATE_TYPE_MESSAGE,
        ...
        if current_round > 0 and random.random() < 0.45:
            generate_random_event(event_cands, current_attempt_prefs,
                                  last_results)
        ...
        if runoff_mode:
            votable_cands = [
                c for c in governor_cands_info
                if c["name"] in runoff_cand_names
            ]
        else:
        ...
        round_gov, results_count, updated_prefs, votes_needed = execute_voting_round(
            ...
            governor_cands_info, carryover_name_round, continue_event,
        ...
        utils.send_pygame_update(
            utils.UPDATE_TYPE_MESSAGE,
            f"Votes needed: {votes_needed} ({current_req_maj*100:.0f}%)")
        ...
        if governor_elected:
            ...
            if not runoff_mode and current_round >= config.MAX_NORMAL_ROUNDS:
                ...
                if not last_normal_results:
                    utils.send_pygame_update(
                        utils.UPDATE_TYPE_ERROR,
                        "No normal round votes. Cannot runoff.")
                    utils.send_pygame_update(utils.UPDATE_TYPE_COMPLETE,
                                             {"elected": False})
                    if running_event:
                        running_event.clear()
                ...
                filtered_res = {
                    c: v
                    for c, v in last_normal_results.items() if c in cands_in
                }
                if len(filtered_res) < 2:
                    utils.send_pygame_update(
                        utils.UPDATE_TYPE_ERROR,
                        "<2 cands with votes. Cannot runoff.")
                    utils.send_pygame_update(utils.UPDATE_TYPE_COMPLETE,
                                             {"elected": False})
                    if running_event:
                        running_event.clear()
                        return
                runoff_cands = [
                    c[0] for c in Counter(filtered_res).most_common(2)
                ]
                runoff_cand_names = runoff_cands
                utils.send_pygame_update(
                    utils.UPDATE_TYPE_MESSAGE,
                    f"Runoff Candidates: {runoff_cands[0]} & {runoff_cands[1]}"
                )
                runoff_mode = True
            ...
            if current_round < config.MAX_TOTAL_ROUNDS and current_round % 3 == 0:
                key_electors = identify_key_electors(current_attempt_prefs,
                                                     last_results)
                if key_electors:
                    utils.send_pygame_update(
                        utils.UPDATE_TYPE_MESSAGE,
                ...
                for idx, ke in enumerate(key_electors):
                    if idx < 5:
                        utils.send_pygame_update(
                            utils.UPDATE_TYPE_MESSAGE,
                            f"  ID: {ke['id']}, Reasons: {', '.join(ke['reasons'])}"
                        )
                    if idx == 5:
                        utils.send_pygame_update(utils.UPDATE_TYPE_MESSAGE,
                                                 "  ...")
        ...
        if election_attempt < config.MAX_ELECTION_ATTEMPTS:
            ...
            if results_carry:
                ...
                if runoff_mode and len(runoff_cand_names) == 2:
                    runoff_res = {
                        c: results_carry.get(c, 0)
                        for c in runoff_cand_names
                    }
                    sorted_runoff = sorted(runoff_res.items(),
                                           key=lambda i: i[1],
                                           reverse=True)
                    if sorted_runoff:
                        ...
                        utils.send_pygame_update(
                            utils.UPDATE_TYPE_MESSAGE,
                            f"Runoff Deadlock. Carrying over: {top_names[0]} & {top_names[1]}"
                        )
                        next_preselected = [
                            c for c in governor_cands_info
                    ...
                    num_carry = max(
                        1, getattr(config, "NUM_PRESELECTED_CANDIDATES", 1))
                    ...
                    if len(next_preselected) == 1:
                        ...
                        utils.send_pygame_update(
                            utils.UPDATE_TYPE_MESSAGE,
                            f"{next_carryover_name} carried over w/ bonus.")
                    else:
                ...
                if next_preselected:
                    utils.send_pygame_update(
                        utils.UPDATE_TYPE_MESSAGE,
                        f"Carried over: {[c['name'] for c in next_preselected]}"
                    )
                else:
                    utils.send_pygame_update(utils.UPDATE_TYPE_MESSAGE,
                                             "No cands carried over.")
            else:
                utils.send_pygame_update(utils.UPDATE_TYPE_MESSAGE,
                                         "No results for carryover.")
            # --- Recursion ---
            utils.send_pygame_update(
                utils.UPDATE_TYPE_MESSAGE,
                f"Initiating Attempt {election_attempt + 1}...")
            time.sleep(1.5)
            run_election_simulation(election_attempt + 1, next_preselected,
                                    next_carryover_name, continue_event,
                                    running_event, step_by_step_mode)
